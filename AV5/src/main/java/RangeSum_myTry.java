...
// TO DO: Implement and run this method from the thread
            long localSum = Arrays.stream(arr).filter(i->i>lower && i<upper).sum();
            lock.lock();
            sum+=localSum;
            if (localSum>maxSum){
                maxSum = localSum;
            ...
            if(localSum==maxSum){
                System.out.println(String.format("Thread with id %s is the max thread , maxC = %d",threadId(),localSum));
            }
