...
public class ElementCount_myTry {
...
// You can add methods or attributes if necessary
    static class CountThread extends Thread {
        ...
        public void countElementsParallel() throws InterruptedException {
            ...
            long localCount =Arrays.stream(arr).filter(i->i==target).count();
...
//            System.out.println(localCount);
            ...
            if(localCount == max ){
                System.out.println("I am thread "+threadId()+" and heres the maximum amount : " + max);
            }
    ...
    static class ArrayGenerator {
        ...
        static int[] generate(int length, int target) {
            ...
            for (int i = 0; i < length; i++) {
                int element = ElementCount_myTry.random.nextInt();
